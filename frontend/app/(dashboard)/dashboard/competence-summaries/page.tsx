...
import { getAllCompetencePapers, getCompetencePaper, deleteCompetencePaper, bulkDeleteCompetencePapers, type CompetencePaperWithCV } from "@/lib/api";
...
export default function CompetenceSummariesPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      clearCachedCompetencePapers();
      await bulkDeleteCompetencePapers(selectedIds);
      await loadPapers(true);
      setBulkDeleteModalOpen(false);
      setSelectedIds([]);
      setIsSelectionMode(false);
    } catch (err: any) {
      setError(err?.message || "Failed to delete selected competence papers.");
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="w-full max-w-sm rounded-xl border border-slate-800 bg-slate-950/95 p-6 shadow-2xl">
            ...
            <p className="text-sm text-slate-400 leading-relaxed mb-4">
              ...
              <span className="font-semibold text-slate-200">{selectedIds.length}</span> selected competence paper{selectedIds.length !== 1 ? "s" : ""}?{" "}

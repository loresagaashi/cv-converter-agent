...
import {
  ...
  bulkDeleteConversationCompetencePapers,
...
export default function ConversationCompetenceSummariesPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      clearCachedConversationPapers();
      await bulkDeleteConversationCompetencePapers(selectedIds);
      await loadPapers(true);
      setBulkDeleteModalOpen(false);
      setSelectedIds([]);
      setIsSelectionMode(false);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Failed to delete selected papers.";
      setError(message);
    } finally {
  ...
  const allSelected = filteredPapers.length > 0 && selectedIds.length === filteredPapers.length;
  const someSelected = selectedIds.length > 0 && selectedIds.length < filteredPapers.length;
  ...
  return (
    <div className="rounded-xl border border-slate-800/60 bg-slate-950/50 p-6 shadow-sm">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="relative flex-1 sm:w-64">
              <input
                ...
                placeholder="Search by CV filename or user"
      ...
      {loading ? (
        ...
        <div
          className={`overflow-x-auto rounded-lg border border-slate-800/60 ${filteredPapers.length > 10 ? "max-h-155 overflow-y-auto" : ""
            }`}
        >
            ...
            <tbody className="divide-y divide-slate-800/60 bg-slate-950/30">
              {filteredPapers.map((paper) => {
                ...
                return (
                  <tr
                    key={paper.id}
                    onClick={isSelectionMode ? () => toggleSelectId(paper.id) : undefined}
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-1 sm:pr-2">
              ...
              {Object.keys(sectionContents).length > 0 ? (
                Object.entries(sectionContents).map(([sectionName, sectionContent]) => (
                  <div
                    key={sectionName}
                    className={`min-w-0 rounded-lg border p-3 shadow-sm transition-all duration-200 sm:p-4 ${editingSection !== null && editingSection !== sectionName
                  ...
                  >
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`inline-flex min-w-[88px] items-center justify-center rounded-lg px-3 py-1.5 text-xs font-semibold transition-all duration-200 ${editingSection === sectionName
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="w-full max-w-sm rounded-xl border border-slate-800 bg-slate-950/95 p-6 shadow-2xl">
            <h3 className="text-base font-semibold text-slate-50 mb-1">
              Delete {selectedIds.length} Paper{selectedIds.length !== 1 ? "s" : ""}
            </h3>
            <p className="text-sm text-slate-400 leading-relaxed mb-4">
              Are you sure you want to permanently delete the{" "}
              <span className="font-semibold text-slate-200">{selectedIds.length}</span> selected conversation paper{selectedIds.length !== 1 ? "s" : ""}?{" "}
